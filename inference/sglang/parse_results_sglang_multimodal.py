...
def main():
    ...
    # Extended regex to capture resolution and image_count too
    pattern = re.compile(
        r"isl(\d+)_osl(\d+)_img_res(\d+)x(\d+)_image_count(\d+)_c(\d+)",
        re.IGNORECASE
    )

    # Select matching JSON files
    file_paths = [
    ...
    # Sort by isl, osl, width, height, image_count, c
    sorted_files = sorted(
        file_paths,
    ...
    for file in sorted_files:
        ...
        isl, osl, width, height, image_count, concurrency = map(int, m.groups())
        ...
        record = {
            ...
            "Input token throughput (tok/s)": data.get("input_throughput"),
    ...
    column_order = [
        "File", "ISL", "OSL", "Image Width", "Image Height", "Image Count",
        "Max concurrency",
        ...
        "Mean TPOT (ms)", "Median TPOT (ms)", "P99 TPOT (ms/token)",
        "Mean ITL (ms/token)", "Median ITL (ms/token)", "P95 ITL (ms/token)", "P99 ITL (ms/token)",
